...
def sheet01():
    ...
    s.note(200, 92,
           ...
           '3. Внешняя полезная нагрузка q_ext = 152,6 кПа; с собственным весом\n'
           '    оболочки, изоляции и пояса q_total = 222,4 кПа. Кольцевое\n'
           '    напряжение 1 308 МПа, запас по пределу прочности 1,53\n'
           '    (п. Б.3, Б.16).\n'
...
def sheet02():
    ...
    s.note(700, 150,
           ...
           '    рёбра, шпангоуты и стрингеры находятся под обшивкой.\n'
           '    Наружная поверхность остаётся гладкой (п. 4.9).\n'
           '7. Сварка автоматическая под флюсом, контроль 100 %.\n'
           '8. Противометеороидный пакет встроен под гладкой обшивкой:\n'
           '    ударный экран, вакуумный зазор, жертвенные слои и\n'
           '    волокнисто-керамический уловитель; наружных рёбер нет.',
           size=7.0)
...
def sheet04():
    ...
    # ── передний причал и крылья батарей ──
    ...
    for yy in (cy-47, cy+47):
        A.add_patch(Rectangle((cx-Lh-45, yy-4), 30, 8, fc='#d9e5f2', ec=C, lw=S_THIN))
        s.line(cx-Lh-15, yy, cx-Lh-7, cy, 'thin')
    A.text(cx-Lh-50, cy-61, 'П · стыковка / батареи / радиаторы', fontsize=6.4,
           ha='left', va='top')

    # ── задняя рама: антенны и собственные двигательные модули ──
    for yy in (cy-48, cy+48):
        s.line(rxp+16, cy, rxp+34, yy, 'thin')
        A.add_patch(Circle((rxp+40, yy), 6.0, fc='white', ec=C, lw=S_THIN))
        s.line(rxp+16, cy, rxp+72, yy, 'thin')
        A.add_patch(Rectangle((rxp+72, yy-5), 18, 10, fc='#d9e5f2', ec=C, lw=S_THIN))
    A.text(rxp+72, cy-61, 'антенны / двигательные модули / батареи', fontsize=6.4,
           ha='left', va='top')
    ...
    # корпус активной магнитной опоры и силовые катушки
    A.add_patch(Rectangle((hx - 6, hy - 26), 30, 52, fc='#eaeaea', ec=C,
                          lw=S_MAIN, zorder=2))
    s.hatch(hx - 6, hy - 26, hx + 24, hy + 26, step=3.0, ang=135)
    for k in (-1, 1):
        A.add_patch(Rectangle((hx + 1, hy + k * 16 - 5), 16, 10,
                              fc='#a9c5dc', ec=C, lw=S_THIN, zorder=5))
        A.text(hx + 9, hy + k * 16, 'М', fontsize=5.8, ha='center', va='center', zorder=6)
    A.text(hx + 9, hy + 38, 'катушки', fontsize=6.2, ha='center')
    # механические аварийные ловители
    for k in (-1, 1):
        A.add_patch(Circle((hx + 9, hy + k * 16), 3.0, fc='white', ec=C,
                           lw=S_THIN, zorder=7))
    # опора на раму
    ...
    s.leader(hx + 9, hy + 16, hx - 30, hy + 44, 'магнитные катушки;\nловители — резерв')
    A.text(hx, hy - 46,
           'Активная магнитная радиально-осевая опора; статор на раме.\n'
           'В том же корпусе — мотор-генератор для раскрутки и торможения.\n'
           'Питание от реакторной шины; боковые оболочки не связаны.',
           fontsize=7.2, ha='center', va='top')
    ...
    legend = ['1 — Подшипниковый узел цапфы, 4 шт.',
              ...
              '4 — Внешняя передняя/задняя рама, оборудование только на ней',
    ...
    s.note(46, 92,
           ...
           '    узлы; боковые поверхности и зазор между корпусами свободны.\n'
           '3. Всё внешнее оборудование находится на рамах: спереди\n'
           '    стыковка, радиаторы и батареи; сзади антенны, радиаторы,\n'
           '    батареи и двигательные модули.\n'
    ...
    s.note(290, 92,
           ...
           '    неподвижна в инерциальном пространстве и не ориентируется\n'
           '    на Солнце; освещение реакторное (п. 11.6).\n'
...
def sheet06():
    ...
    s.table(46, 232,
            ...
            [['Место размещения', 'L1 Земля — Солнце', 'L1, решение сохранено', '11.5'],
             ...
             ['Ориентация оси', 'на Солнце', 'фиксирована; на Солнце не ориентируется', '11.6'],
...
def sheet07():
    ...
    s.note(300, 268,
           ...
           '8. Передний переход: шлюз — вращающееся гермоуплотнение —\n'
           '    полый осевой вал — две наклонные шахты цилиндра.\n'
           '9. В каждом торцевом узле: магнитный радиально-осевой\n'
           '    подшипник, мотор-генератор и механический ловитель;\n'
           '    питание — от реакторной шины неподвижной рамы.\n'
           '10. Противометеороидные экраны встроены под гладкую\n'
           '     наружную обшивку и не образуют наружных рёбер.',
           size=7.2)
