...
def antennas(ex, hy, side, K):
    """Антенны на кормовом узле, тарелки смотрят назад (к Земле)."""
    d=max(DISH*K,2.4)
...
T(60,1314,'корпуса ничего не касаются; связь только по осям', size=8.4,
  color=DIM)
...
T(xc+hl+CEND*K+AXLEN*K+90, yA+dyc+95, 'НОС — к Солнцу', size=9, w='bold',
  color='#ffd98a')
T(xc-hl-CEND*K-AXLEN*K-90, yA+dyc+95, 'КОРМА — к Земле', size=9, w='bold',
  color='#9fd0ff', ha='right')

lead(xc+hl+CEND*K*0.4, yA+dyc+R*K*0.75, xc+hl-40, yA+dyc+R*K+92)
T(xc+hl-36, yA+dyc+R*K+96, 'носовой купол целиком в фотоэлементах', size=8)
...
T(xc-hl-256, yA+dyc-154, 'антенны — тарелки на Землю', size=8, ha='left')
...
# ════════════════════ ВИД B: СПЕРЕДИ ════════════════════
T(1250,1330,'B.  ВИД СПЕРЕДИ (со стороны Солнца)', size=13, w='bold')
...
T(xB, yB-dB-rB-46, 'балка соединяет только узлы', size=8, color=DIM,
  ha='center')
T(xB+rB+60, yB, 'вся передняя\nполусфера —\nфотоэлементы', size=8.2,
  color='#9fc4ff')
...
T(xCv+64, yCv-219, 'фотоэлементы купола', size=8)
...
T(ox, oy-ORB-70,
  'Полгода Солнце светит в нос, полгода — в корму.', size=8,
  color=DIM, ha='center')
T(ox, oy-ORB-86,
  'ПОЭТОМУ ПАНЕЛЯМИ ПОКРЫТЫ ОБА КУПОЛА: 53,4 ГВт круглый год,',
  size=8.4, color='#9fc4ff', ha='center', w='bold')
T(ox, oy-ORB-100,
  'без подвижных частей и без разворота корпуса.', size=8.4,
...
for i,(k,v) in enumerate([
    ('поверхность купола','146,7 км² · на 4 купола 586,8 км²'),
    ('проекция на луч','157,1 км² — не зависит от угла'),
    ...
    ('привод секций узла','0,986 °/сут — оборот за год')]):
...
for i,(k,v) in enumerate([
    ...
    ('подшипниковый узел','на конце цапфы, не вращается'),
    ('балка','соединяет узлы; поверхности не касается'),
    ('нос (к Солнцу)','стыковка · радиаторы · панели'),
    ('корма (к Земле)','антенны · радиаторы · панели'),
...
T(1840,60,'Бугаенко Р. С. · НИК   ·   ред. 7.0, 08.09.2026   ·   '
          'verify_all.py 241/241', size=8, color=FAINT, ha='right')
